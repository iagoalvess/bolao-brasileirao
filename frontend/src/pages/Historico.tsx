...
function calcularPontuacao(resultado: {home: number, away: number}, palpite: {home: number, away: number}) {
  if(resultado.home === palpite.home && resultado.away === palpite.away) return 10;
  if(resultado.home - resultado.away === palpite.home - palpite.away) return 5;
  if((resultado.home > resultado.away && palpite.home > palpite.away) ||
     (resultado.home < resultado.away && palpite.home < palpite.away) ||
     (resultado.home === resultado.away && palpite.home === palpite.away)) return 3;
  return 0;
}
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center text-red-600 py-6">
                  {(error as any).response?.data?.detail || "Erro ao carregar histórico."}
                </div>
                          ...
                          <TableRow key={match.id}>
                            ...
                            <TableCell>
                              {resultadoFinalizado && match.scoreboard
                                ...
                                : <span className="text-xs text-gray-500">Não finalizado</span>
                              }
                            </TableCell>
                            <TableCell>
                              {palpite
                            ...
                            <TableCell>
                              {palpite && resultadoFinalizado && match.scoreboard
                                ? calcularPontuacao(
                                  { home: match.scoreboard.home, away: match.scoreboard.visitor },
                                  { home: palpite.home_team_score, away: palpite.away_team_score }
                                  )
                                : "--"
                              }
                            </TableCell>
                ...
                <div className="text-center py-6">Nenhum histórico de partidas encontrado.</div>
              )}
            </CardContent>
          </Card>
        </div>
