...
const Matches = () => {
  ...
  const [predictions, setPredictions] = useState<{[key: number]: {home_team_score: string, away_team_score: string}}>({});
  ...
  useEffect(() => {
    if (myPredictions && Array.isArray(myPredictions)) {
      const mapped: {[key: number]: {home_team_score: string, away_team_score: string}} = {};
  ...
  const handleChange = (matchId: number, field: "home_team_score" | "away_team_score", value: string) => {
    setPredictions(prev => ({
      ...prev,
      [matchId]: {
        ...prev[matchId],
  ...
  const handlePalpite = (match: Match) => {
    ...
    if (!input || input.home_team_score === "" || input.away_team_score === "") {
      toast({
        ...
        <div className="container mx-auto max-w-3xl">
          <Card className="shadow-lg border-2 border-soccer-yellow mb-8">
            <CardHeader className="flex items-center justify-between">
               <CardTitle className="text-2xl">Partidas de hoje</CardTitle>
               <Button variant="secondary" size="sm" onClick={() => navigate("/historico")}>
                 Ver Histórico
               </Button>
             </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center text-red-600 py-6">
                  {(error as any).response?.data?.detail || "Erro ao carregar partidas."}
                </div>
                        ...
                        <TableRow key={match.id}>
                          ...
                          <TableCell>
                            <div className="flex gap-1 items-center">
                              <input
                                ...
                                onChange={e => handleChange(match.id, "home_team_score", e.target.value)}
                                className="w-10 rounded-md border border-gray-300 text-center"
                                maxLength={2}
                                disabled={match.status === "FINISHED" || mutation.isPending || predicted}
                              />
                              ...
                              <input
                                ...
                                onChange={e => handleChange(match.id, "away_team_score", e.target.value)}
                                className="w-10 rounded-md border border-gray-300 text-center"
                                maxLength={2}
                                disabled={match.status === "FINISHED" || mutation.isPending || predicted}
                              />
                          ...
                          <TableCell>
                            ...
                            {match.status === "FINISHED" && (
                              <span className="inline-block ml-2 text-xs text-gray-500">Finalizado</span>
                            )}
                          </TableCell>
                        </TableRow>
                      );
